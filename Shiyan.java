...
{	 //匹配规则
	...
	static Pattern pattern1 = Pattern.compile("^[1-9a-z](\\*[1-9a-z])*(\\+[1-9a-z](\\*[1-9a-z])*)*");
	static Pattern pattern2 = Pattern.compile("^[!](simplify)(\\s[a-z]\\=[]0-9]+)+");
        ...
        else if(matcher2.matches() && flag == 1)
        {
        	...
        	for(i=1;i<str2.length;i++)
        	{
        		if(char_in_not(str1,str2[i].substring(0,1)) == 1)
        		{
        			...
        			{
        				str1[j] = str1[j].replace(str2[i].substring(0,1),str2[i].substring(2,str2[i].length()));
        			}
        		}
        	}
        	if(f==0)
	        	...
	        	for(j=0;j<str1.length;j++)
	        	{
	        		...
	        		else
	        		{
	        			...
	        			for(k=0;k<str2.length;k++)
	        			{
	        				...
	        				str1[j] = str1[j].replaceFirst(x, (String)(num+""));
